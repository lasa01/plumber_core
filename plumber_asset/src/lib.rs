...
use crate::{
    mdl::{LoadedModel, Loader as ModelLoader, ModelInfo, Settings as MdlSettings},
    vmf::{LoadedProp, PropError, Settings as VmfSettings},
    vmt::{
        LoadedMaterial, LoadedTexture, LoadedVmt, Loader as MaterialLoader, MaterialLoadError,
        SkyBox, TextureLoadError,
    },
};
...
mod tests {
    ...
    #[test]
    fn scene_loading() {
        let vmf = include_bytes!("../tests/build_scene_test.vmf");
        ...
        vmf::load(&parsed, importer, &VmfSettings::default(), || ());
    }
}
